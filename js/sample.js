        ...
        $.getJSON(feedUrlF, function (data) {
           ...
           if(storyData!=''){
            ...
            if(rflTop && rflTopFlag && idx == 2){
                ...
                if(hload == 1){var tmOut=2000;}else{var tmOut=0;
...
$(window).scroll(function () {
    ...
    if(!$('#'+iframeId).isOnScreen() && vdoFlag==1){console.log($('.videoLi').isOnScreen());console.log(vdoFlag);
        if(iOS){
    ...
    if(!$('#'+iframeId).isOnScreen() && vdoFlagv==1){console.log($('.videoLi').isOnScreen());
        pauseVideoUltima(2);   
    }
});
...
var iframeId="dataIframe";
 $(document).on("click",".videoContainer,.videoTitle",function(){
    ...
    if(iOS){ setTimeout(function(){ 
        $('#'+iframeId).css({'top':videoPos,'width':videoWidth,'height':videoHeight}).show(); }, 300); }else{
...
function _bhaskarOneResponseCallback(e) {
            ...
            if(e.action.type=="_onClick"){console.log('_onClick on element:'+e.action.element);
...
$(document).on("visibilitychange", function() { if(document.hidden){
        responsiveVoice.cancel();
    /*responsiveVoice.pause();*/
        $("#resume").show();
        $(".pausevoice").hide();
